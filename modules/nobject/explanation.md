...
**Ý Nghĩa**:
    Module `nobject` chịu trách nhiệm định nghĩa kiến trúc Object cơ bản và điều phối vòng đời logic (`OnStep`) cũng như xử lý Input callback của các đối tượng trong game.
...
**Chức năng chính**:
    ...
    3. **LogicSystem (LogicSystem.go)**:
        Hệ thống điều phối logic. Nó không chứa dữ liệu mà chỉ thực hiện việc duyệt qua danh sách các Object và kích hoạt sự kiện `OnStep`.
...
**Quy trình hoạt động**:
    1. **Update Flow** (trong Scene.Update):
       ...
       LogicSystem.Update(objectList) → gọi OnStep trên từng Object
    2. **Duyệt Object**:
       Hệ thống nhận danh sách `IObject` và lặp qua từng phần tử.
    3. **Kích hoạt Logic**:
       Gọi hàm `OnStep()` trên từng Object để thực thi các kịch bản logic cụ thể của đối tượng đó.